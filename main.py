...
def forward_and_visual(mod, img):
...
if __name__ == "__main__":
    ...
    mod = get_model(args.model)
    mod.load_weights(args.weights)

    img = cv2.imread(args.image)
    img = cv2.resize(img, (224, 224))
    img = np.array(img, np.float32) / 255.
    img -= [0.16718367, 0.16718367, 0.16718367]
    img /= [0.32906374, 0.33593044, 0.38852182]

    img = img.reshape((1,) + img.shape)

    print(mod.predict(img))

    forward_and_visual(mod, img)    